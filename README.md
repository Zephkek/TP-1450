...
I have been down a big rabbit hole which is finding vulnerabilities in this router which i thought ill only find a few but there are so many flaws in this router that highlight signficant firmware problems, resource management and memory management problems.
Today I discovered an intricate and interesting memory corruption vulnerability that resides in TP-Link's VN020-F3v(T) which is a custom implementation that's limited and propriatary that's made by TP-Link and customized for ISPs across north africa in here affecting many countries such as Tunisia, Algeria and morocco which have this same model being sold on their markets actively. This vulnerability exposes critical security weaknesses through specially crafted FTP payload demonstrating very deep issues within this router. 
